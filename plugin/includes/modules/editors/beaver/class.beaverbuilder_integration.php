...
class BeaverBuilder_Integration extends \TMA\ExperienceManager\Integration {
	...
	function custom_attributes($attrs, $module) {
		//if (property_exists($module->settings, "tma_personalization") && $module->settings->tma_personalization === "enabled") {
		if ($this->isActivated($module)) {
			...
			if (!\FLBuilderModel::is_builder_active() && !tma_exm_beaver_admin_bar_visible()) {
				if (tma_exm_is_frontend_mode_enabled()) {
					if ($module->settings->tma_default === "no") {
	...
	protected function isActivated($module) {
		return (property_exists($module->settings, "tma_personalization") && $module->settings->tma_personalization === "enabled");
	}
